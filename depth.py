...
sample = []
print("盤面を入力してください(空洞は0)")   # 問題の手動入力
for i in range(SIZE):
    sample.append([])
for i in range(SIZE):
...
prelist=copy.deepcopy(closedlist)
times=0

print("探索開始")
while(closedlist.S!=GOAL):
    closedlist=copy.deepcopy(openlist[0])                        # deepcopyで上書きする
    closedlist.pre=copy.deepcopy(prelist)
    print(f"{times}回目：{closedlist.S} (d={closedlist.depth})")  # 途中経過の表示
    ...
    openlist.sort(reverse=True,key=lambda u:u.depth)  # 深さで降順ソート
    while(openlist[0].depth>31):                      # 8パズルは理論上31手までで攻略できるため操作回数に制限を設ける
        openlist.pop(0)
    closedlist = copy.deepcopy(openlist[0])
    times+=1
